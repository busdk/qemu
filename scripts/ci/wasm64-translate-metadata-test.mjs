...
assert.match(tci, /tci_wasm_generated_signature\(code_start,\s*tb_start,\s*code_ops\)/);
assert.match(tci, /EM_JS\(uintptr_t,\s*tci_wasm_generated_compile_js,\s*\n\s*\(uintptr_t code_arg,\s*uintptr_t relative_base_arg/);
assert.match(tci, /const code = Number\(code_arg\);/);
assert.match(tci, /const relativeBase = Number\(relative_base_arg\);/);
assert.match(tci, /const insnPtr = code \+ index \* 4;/);
assert.match(tci, /const tbPtr = relativeBase \+ index \* 4 \+ 4;/);
assert.match(tci, /QEMU_TCI_WASM_GENERATED_TRACE/);
assert.match(tci, /qemu-tci-wasm-generated-trace:/);
assert.match(tci, /tci_wasm_generated_trace_event\("compile"/);
assert.match(tci, /tci_wasm_generated_trace_event\("exec-dispatch"/);
assert.match(tci, /tci_wasm_subset = tci_parse_bool_env\(raw\);/);
assert.doesNotMatch(tci, /raw == NULL \? true : tci_parse_bool_env\(raw\)/);
assert.match(tci, /functionType\(\[valueI64\], \[valueI64\]\)/);
assert.match(tci, /addFunction\(instance\.exports\.run, "jj"\)/);
assert.doesNotMatch(tci, /addFunction\(instance\.exports\.run, "ii"\)/);
assert.match(tci, /tcg_wasm64_translate_generated_output_available\(metadata\)/);
assert.match(tci, /code_start = metadata->generated_output;/);
assert.match(tci, /code_ops = metadata->generated_output_op_count;/);
assert.match(tci, /tci_wasm_generated_try_exec\(entry,\s*tb_start,\s*code_start,\s*\n\s*code_ops,\s*regs,\s*ret,\s*true\)/);
assert.match(tci, /tci_wasm_generated_try_exec\(entry,\s*tb_start,\s*tb_start,\s*\n\s*tci_wasm_subset_max_ops,\s*regs,\s*ret,\s*\n\s*false\)/);
