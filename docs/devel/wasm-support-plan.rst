...
R4a deterministic run-loop benchmark gate
-----------------------------------------

R4a adds a cheap deterministic preflight for the generated run-loop before
another expensive browser speed gate is attempted.  The gate is intentionally
narrow: it compares the current generated ``wasmjit_run()`` micro-workloads
against the local TCI-like interpreter model for the ALU/branch and TLB-hit RAM
paths.  Passing it does not prove Linux boot acceleration.  Failing it would
make a browser speed gate pointless because the run-loop shape would not yet
beat the fallback model on the workloads it is designed to accelerate.
...
.. code-block:: console
  ...
  $ node scripts/ci/wasmjit-runloop-benchmark-gate.mjs \
      --out /Users/test/git/busdk/agent-supervisor/tmp/qemu-r4-wasmjit-preflight-local/runloop-benchmark-gate.json

Result JSON SHA-256:
``8deed1a988f7f778792a93f7e9db06fc1a5ea512e472246a97addc1f22861849``.

The archived result used budget ``1000000``, rounds ``7``, and the default
minimum ratio ``3``.  ``alu-branch`` passed with wasm best
``0.38574999999999804`` ms, TCI-like best ``34.14274999999998`` ms, ratio
``88.51004536617019``.  ``tlb-hit-ram`` passed with wasm best
``1.9760410000000093`` ms, TCI-like best ``64.48566600000004`` ms, ratio
``32.6337692385936``.

The same slice also passed a full ``riscv64-softmmu`` backend artifact compile
gate:

.. code-block:: console

  $ python3 scripts/ci/wasm-build-artifacts-local.py \
      --out /Users/test/git/busdk/agent-supervisor/tmp/qemu-riscv64-wasm-runloop-preflight \
      --target riscv64 --tcg-wasm64-backend --build-image
...
This is the R4 preflight only.  The next required step is an in-binary
Emscripten runtime microbench that compares ``wasmjit_run()`` with the
C/TCI-like hotset interpreter path inside the QEMU/WASM artifact, followed by
the same-commit generic Chromium speed gate.
...
R4c failed same-commit generic speed gate
-----------------------------------------

The first R4c same-commit speed gate did not pass.  It is retained here as
negative evidence and a replanning point.

The default-TCI artifact from commit
``9ebd2fcc7e7727564a3be86721ae7a0b1f9af3b3`` had these SHA-256 values:
...
Chromium ``149.0.7827.55`` reached ``Welcome to TuxTest`` in ``127204`` ms.
The result JSON was
...
The backend reached the same marker in ``140286`` ms.  The result JSON was
...
``13082`` ms slower than default TCI, with backend/default ratio
``1.1028426779032106``.  This rejects the current structural runtime smoke as
a boot-speed fix.  It does not reject the accelerator architecture; it shows
that translated Linux TBs are not yet executing through the long-running
``wasmjit_run()`` path.

R4d bounded backend translation telemetry
-----------------------------------------

R4d adds bounded, opt-in backend translation summaries for real generic
RISC-V browser smokes.  The expected effect on the gate metric was diagnostic,
not a speed improvement: expose whether translated TB output exists during a
normal boot, and whether generated Linux execution remains zero.  The new
controls are ``--wasm64-tcg-summary``,
``--wasm64-tcg-summary-interval``, and
``--wasm64-tcg-summary-limit``.
...
.. code-block:: console
  ...
  $ python3 scripts/ci/wasm-build-artifacts-local.py \
      --out /home/coding-agent/coding-agent/git/busdk/agent-supervisor/tmp/qemu-r4d-riscv64-tcg-summary-limited-artifacts \
      --target riscv64 \
      --tcg-wasm64-backend \
      --jobs auto \
      --build-image
...
.. code-block:: console
  ...
  $ npm exec --yes --package=playwright -- node \
      scripts/ci/wasm-browser-smoke-runner.mjs \
      --artifact-dir /home/coding-agent/coding-agent/git/busdk/agent-supervisor/tmp/qemu-r4d-riscv64-tcg-summary-limited-artifacts \
      --guest-manifest /home/coding-agent/coding-agent/git/busdk/agent-supervisor/tmp/qemu-r3e-riscv64-guest/tuxboot-browser-smoke-guest.json \
      --out /home/coding-agent/coding-agent/git/busdk/agent-supervisor/tmp/qemu-r4d-riscv64-tcg-summary-limited-smoke/wasm-browser-smoke-result.json \
      --screenshot /home/coding-agent/coding-agent/git/busdk/agent-supervisor/tmp/qemu-r4d-riscv64-tcg-summary-limited-smoke/wasm-browser-smoke.png \
      --timeout-ms 180000 \
      --progress-sample-interval-ms 10000 \
      --progress-sample-limit 40 \
      --wasm64-tcg-summary \
...
It reached ``Welcome to TuxTest`` in ``140671`` ms.  Result JSON SHA-256:
...
Boot milestones were first Linux printk at ``35399`` ms, root block discovery
at ``39777`` ms, rootfs mounted at ``51142`` ms, and init started at
``52040`` ms.
...
``51.92%``.  The generated-candidate supported-op share was ``97.37%``.
...
``call=20350``, ``not=108``, ``sar=106``, ``divu=81``, ``br=36``,
``remu=22``, ``divs=18``, and ``mulu2=14``.

The accepted conclusion is that real translation-time output material is
present during generic boot, but no generated Linux TBs are compiled or
executed yet.  The next accelerator item is attaching eligible
translated-output hotsets to the long-running ``wasmjit_run()`` executor with
...
R4f attach-failure attribution
------------------------------

R4f keeps the R4e probe diagnostic-only and adds the attribution needed before
broadening the descriptor ABI.  The expected gate effect was no speed
improvement.  The diagnostic splits ``unsupported_hot_tb`` into generated-op
rejections, semantic descriptor-shape rejections, and other rejections, and it
reports the top first TCI op for semantic-shape rejects.

The first R4f artifact was rejected because the semantic first-op histogram was
kept in static thread-global storage and could exceed
``unsupported_semantic_shape`` after summary aggregation.  The accepted
implementation keeps the histogram in ``TCGWasm64Counters`` and merges it
through the normal counter aggregation paths.
...
.. code-block:: console
  ...
  $ python3 scripts/ci/wasm-build-artifacts-local.py \
      --out /home/coding-agent/coding-agent/git/busdk/agent-supervisor/tmp/qemu-r4f-riscv64-attach-attribution-v2-artifacts \
      --target riscv64 \
      --tcg-wasm64-backend \
      --jobs auto \
      --build-image
...
The browser diagnostic used Chromium ``149.0.7827.55``:

.. code-block:: console

  $ npm exec --yes --package=playwright -- node \
      scripts/ci/wasm-browser-smoke-runner.mjs \
      --artifact-dir /home/coding-agent/coding-agent/git/busdk/agent-supervisor/tmp/qemu-r4f-riscv64-attach-attribution-v2-artifacts \
      --guest-manifest /home/coding-agent/coding-agent/git/busdk/agent-supervisor/tmp/qemu-r3e-riscv64-guest/tuxboot-browser-smoke-guest.json \
      --out /home/coding-agent/coding-agent/git/busdk/agent-supervisor/tmp/qemu-r4f-riscv64-attach-attribution-v2-smoke/wasm-browser-smoke-result.json \
      --screenshot /home/coding-agent/coding-agent/git/busdk/agent-supervisor/tmp/qemu-r4f-riscv64-attach-attribution-v2-smoke/wasm-browser-smoke.png \
      --timeout-ms 180000 \
      --progress-sample-interval-ms 10000 \
      --progress-sample-limit 40 \
      --wasm64-tcg-summary \
...
It reached ``Welcome to TuxTest`` in ``145265`` ms.  Result JSON SHA-256:
``bd816e344e9ca83c97dc028f11061fc66d1eede4b54ba95191da9fe42b70c316``.
Screenshot SHA-256:
``62325f7c3d749a4efa8a683a015664c8951cf4be1ba5c194519e43ee54655343``.

The final summary still reported zero generated attempts, compiled blocks, or
executed blocks, so this is not a speed-gate result.  It reported
``translated_tbs=42614``, ``translated_ops=1494226``,
...
``exec_generated_output_available_tbs=22346``.

The attach probe sampled ``200000`` TB entries and found ``ready_tbs=0``,
``ready_ops=0``, ``missing_metadata=157386``,
``unsupported_hot_tb=42614``, ``unsupported_generated_ops=20268``,
``unsupported_semantic_shape=22346``, ``unsupported_other=0``,
``no_generated_output=0``, and ``output_truncated=0``.  The semantic first-op
histogram sums exactly to ``unsupported_semantic_shape``: ``ld32u=21968`` and
``tci_movi=378``.  The generated-op rejection histogram is dominated by
``call=20109``, followed by ``not=107``, ``sar=107``, ``divu=81``,
``br=35``, ``remu=21``, ``divs=18``, and ``mulu2=12``.

The accepted conclusion is that the next accelerator implementation should not
run another browser speed gate yet.  It should first teach the run-loop
descriptor path to represent load-first translated-output shapes dominated by
...
expected-value calculation.  The speed evidence is recorded in the R4a and
W2o-b microbench gate sections; W2o-a is the ABI contract slice.
...
.. code-block:: text
  ...
  alu-branch:
    wasmBestMs=0.38574999999999804
    tciLikeBestMs=34.14274999999998
    bestRatio=88.51004536617019

  tlb-hit-ram:
    wasmBestMs=1.9760410000000093
    tciLikeBestMs=64.48566600000004
    bestRatio=32.6337692385936
...
The full backend artifact compile gate also passed with:

.. code-block:: text

  python3 scripts/ci/wasm-build-artifacts-local.py --out /Users/test/git/busdk/agent-supervisor/tmp/qemu-riscv64-wasm-runloop-preflight --target riscv64 --tcg-wasm64-backend --build-image

The produced hashes were ``qemu-system-riscv64.js``
``c398c19673c3de5c3d4331cda83ba48b14470627c582072385e751cf75e9ff3f``,
``qemu-system-riscv64.wasm``
``60ea302e6600f4dfd0637a7ca5df9dbff30fedd3e2de655f7a709cb978b46959``, and
``qemu-system-wasm-artifacts.json``
``c0d15b7d992acfbb49e79f7f31bbdf7a7deeffd24826838dcf3b50a0a96ea82a``.
