...
the active RISC-V target architecture. For this Bus Engine OS browser goal,
read these as the `riscv64-softmmu` accelerator and accepted Bus Engine OS
`riscv64` `virtual-server` kernel/rootfs. x86_64 work is limited to explicit
non-regression checks unless the operator opens a separate x86 lane.
...
- [x] R1e - Make the browser smoke runner consume browser-hosted manifests
  ...
  Accepted 2026-07-06 22:24 EEST: commit `960b7409af` adds nested
  `default_parameters` normalization, manifest-derived artifact/firmware
  directories, `targetArch` runner/URL/result plumbing, and blank-initrd
  normalization. Checks passed: `node --check
  scripts/ci/wasm-guest-manifest.mjs`, `node --check
  scripts/ci/wasm-browser-smoke-runner.mjs`, `node
  scripts/ci/wasm-guest-manifest-test.mjs`, `node
  scripts/ci/wasm-browser-smoke-runner-test.mjs`, and `git diff --check`.
  A manifest-only 5-second browser probe using
  `/private/tmp/beo-browser-policy-timing-20260706b/browser-hosted-manifest.json`
  wrote `/private/tmp/qemu-runner-manifest-parity-20260706a-probe2.json`,
  reached OpenSBI/RISC-V output in Chrome/Playwright with `64` guest lines,
  used `targetArch=riscv64`, `-M virt`, blank CPU, rootfs and persistent disks
  from the manifest, no duplicate `-append`, and no manual `-bios`,
  `--machine`, `--cpu`, or rootfs/display overrides.
...
- [ ] R4d-g - Diagnose and repair the current RV64 generated-exec
    ...
    proof.
    Progress 2026-07-07 (commit `6e92bf7fc7`): fixed both gate-contract
    blockers with deterministic reproductions. The CDP proof gate now accepts
    positive `generated_coverage_numerator=163` and
    `generated_coverage_denominator=964` without requiring a literal
    `generated_coverage_ppm`, deriving `169087` ppm when needed. The x86
    metrics gate now treats an empty optional `wasm64Tcg={}` placeholder as
    absent unless `--require-tcg` is set, while still failing genuinely broken
    TCG summaries. Verification passed: `node --check
    scripts/ci/wasm-browser-cdp-proof-gate.mjs`, `node --check
    scripts/ci/wasm-browser-cdp-proof-gate-test.mjs`, `node
    scripts/ci/wasm-browser-cdp-proof-gate-test.mjs`, `node --check
...
- [x] R4s16 - Repair the x86 live-generated-exec chain-target stop before
    ...
    either committed with an explicit synthetic chain-target exit and preserved
    generated-retirement counters in no-silent mode, or the body is rejected
    before execution with a precise fail-closed reason. The fix must not hide
    the continuation in TCI fallback, must keep unsupported target execution
